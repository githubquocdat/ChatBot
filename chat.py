...
def get_response(chat,slots,count):
    ...
    if prob_state > 0.75 and count[0] == 0:
        ...
        slots.clear()
        slots.insert(1, state)
    if len(slots) == 1 and tag != "greeting" and prob_intent > 0.75 and slots[0] in list_key_intent:
        slots.clear()
    if prob_intent > 0.75: #du doan intent dung nen them vao slot
        slots.insert(0, tag)
    if len(slots) == 3 and slots[0] == "greeting": #khi chi nhap ten nganh, mac dinh intent = greeting duoc them vao bi du nen phai xoa
        slots.pop(0)
    print(slots)
    if len(slots) == 1: #thieu state hoặc intent
            ...
            elif slots[0] in list_key_db: #thieu state, intent (học phí, giới thiệu chung, điểm chuẩn, tổ hợp xét tuyển)
        ...
        if slots[0] in list_key_db: #intent (học phí, giới thiệu chung, điểm chuẩn, tổ hợp xét tuyển)
        ...
        elif slots[0] == "greeting" and slots[1] in state_tags: #khi chi nhap ten nganh, mac dinh intent: greeting
            str = f"Bạn cần hỏi thông tin nào của ngành {slots[1]}?"
            slots.pop(0)
            return str
        elif slots[0] not in list_key_db and slots[1] in state_tags: #intent không phải (học phí, giới thiệu chung, điểm chuẩn, tổ hợp xét tuyển), slot 2 khong phai state
            for intent in intents["intents"]:
                if slots[0] == intent['tag']:
                    slots.pop(0)
                    return f"{random.choice(intent['responses'])}"
